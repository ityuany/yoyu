...
规则测试覆盖股票数量乘单价、市值四舍五入与溢出保护、年化收益率正负值、金额精度与范围、退休年龄、2026 年全部 33 个放假日期及 6 个补班日期、个人覆盖优先级和未知年份降级。持久化测试使用独立临时 SwiftData 数据库，覆盖保存后重新打开、旧股票金额兼容、新股票字段及年化收益率保存、空值、回滚与移除个人调整，不访问业务数据库或 iCloud。

当前项目部署目标为 iOS 27，本机需使用 Xcode-beta.app 构建；无需修改全局 xcode-select。Debug 构建可传 `--profile` 直接打开「我的」，附加 `--holidays` 查看调休详情，或 `--edit 基本信息`（也支持「企业信息」「工作安排」「当前财富」）查看编辑页。Release 不使用这些参数。
